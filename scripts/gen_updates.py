...
(first-add date), and writes:
  ...
  - brain/KEYWORDS.md                   — keyword → entry map: one line per
    key across all entries (keys verbatim, sorted; a key shared by several
    entries lists all targets on one line)

Usage:
  python3 scripts/gen_updates.py            # regenerate all files in place
  python3 scripts/gen_updates.py --check    # exit 1 if any file is stale;
                                            # also WARNS (not fails) when the
                                            # entry count passes the INDEX
                                            # split threshold (CONTRIBUTING.md
                                            # "Scaling the INDEX")
...
# Above this many brain entries, --check warns that the single INDEX.md
# should be split per-domain (CONTRIBUTING.md "Scaling the INDEX").
...
def collect_keyword_map(repo_root: Path) -> "dict[str, list[str]]":
    """Map each key (verbatim) to the brain-relative entry paths carrying it."""
    brain = repo_root / "brain"
    ...
    for path in sorted(brain.rglob("*.md")):
        ...
        for key in parse_entry(path).get("keys") or []:
            targets = mapping.setdefault(str(key), [])
            if rel not in targets:
                targets.append(rel)
    return mapping


def render_keywords(mapping: "dict[str, list[str]]") -> str:
    """brain/KEYWORDS.md text: header + one sorted line per key."""
    lines = [
        "%s → %s" % (key, " · ".join(sorted(mapping[key])))
        for key in sorted(mapping)
    ]
    return KEYWORDS_HEADER + "\n".join(lines) + "\n"


def index_scale_warning(entry_count: int,
                        threshold: "int | None" = None) -> "str | None":
    """WARNING text once the brain outgrows a single INDEX.md, else None."""
    if threshold is None:
        threshold = INDEX_SPLIT_THRESHOLD
    if entry_count <= threshold:
        return None
    return ("WARNING: %d brain entries exceeds the single-INDEX threshold (%d) "
            "— time to split INDEX.md per domain; see \"Scaling the INDEX\" "
            "in CONTRIBUTING.md" % (entry_count, threshold))
...
def main(argv: "list[str] | None" = None) -> int:
    ...
    keywords_path = repo_root / "brain" / "KEYWORDS.md"
    new_json = render_json(data)
    new_keywords = render_keywords(collect_keyword_map(repo_root))
    ...
    if args.check:
        warning = index_scale_warning(len(entries))
        if warning:
            print(warning)
        old_json = updates_path.read_text(encoding="utf-8") if updates_path.exists() else ""
        old_keywords = (keywords_path.read_text(encoding="utf-8")
                        if keywords_path.exists() else "")
        stale = []
        ...
        if old_keywords != new_keywords:
            stale.append(_diff_summary("brain/KEYWORDS.md", old_keywords, new_keywords))
        if stale:
    ...
    keywords_path.write_text(new_keywords, encoding="utf-8")
    print("wrote %s + %s and patched %s + %s (%d entries, %d skills, %d update rows)" % (
        updates_path, keywords_path, index_path, readme_path,
        data["stats"]["entries"], data["stats"]["skills"], len(data["updates"])))
    return 0
