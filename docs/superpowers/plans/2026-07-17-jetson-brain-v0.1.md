...
- `python3 scripts/lint_brain.py ~/.jetson-brain/local` reports no frontmatter/schema
  errors; a single `INDEX.md missing` error (and exit code 1) is expected for the
  overlay — the distill flow never creates an overlay INDEX — and does not fail
  this scenario.
